...
*         lang      = '-D'
  ...
  method get_guid.
    ...
    try.
        rv_return = cl_system_uuid=>create_uuid_x16_static( ).
      catch cx_uuid_error into data(oref).
        rv_return = '0'.
    endtry .
  ...
  method on_added_function .
    ...
    case e_salv_function .
      ...
      when 'REPROCESS' .
        ...
        data(rows) = go_salv_table->get_selections( )->get_selected_rows( ).
        data(row) = value #( rows[ 1 ] optional ).
        data(ls_outtab) = value #( me->gt_outtab[ row ] optional ) .

        if ( ls_outtab is not initial ) .

          "data(ls_zppt0004) = value #( me->gt_zppt0004[ pknum   = ls_outtab-pknum
